"""HeteroFL method implementation using shared task code."""
...
from .strategy import HeteroFLStrategy
...
def train_app(msg: Message, context: Context) -> Message:
    return client_train(msg, context, method_label="heterofl", resolve_model_rate=_fallback_model_rate)


def evaluate_app(msg: Message, context: Context) -> Message:
    return client_evaluate(msg, context, method_label="heterofl", resolve_model_rate=_fallback_model_rate)
...
def run_server(grid: Grid, context: Context) -> None:
    run_server_loop(
        ...
        strategy_factory=HeteroFLStrategy,
