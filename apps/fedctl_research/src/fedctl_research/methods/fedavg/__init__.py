"""FedAvg baseline method implementation using shared task code."""
...
from .strategy import FedAvgBaseline
...
def train_app(msg: Message, context: Context) -> Message:
    return client_train(msg, context, method_label="fedavg", resolve_model_rate=_global_model_rate)


def evaluate_app(msg: Message, context: Context) -> Message:
    return client_evaluate(msg, context, method_label="fedavg", resolve_model_rate=_global_model_rate)
...
def run_server(grid: Grid, context: Context) -> None:
    run_server_loop(
        ...
        context,
        method_label="fedavg",
        strategy_factory=FedAvgBaseline,
