"""FedRolex method implementation using shared task code."""
...
def train_app(msg: Message, context: Context) -> Message:
    return client_train(msg, context, method_label="fedrolex", resolve_model_rate=_fallback_model_rate)


def evaluate_app(msg: Message, context: Context) -> Message:
    return client_evaluate(msg, context, method_label="fedrolex", resolve_model_rate=_fallback_model_rate)
...
def run_server(grid: Grid, context: Context) -> None:
    run_server_loop(
        grid,
        context,
        method_label="fedrolex",
        strategy_factory=FedRolex,
        ...
        roll_mode=(get_optional_str(context.run_config, "fedrolex-roll-mode") or "paper").lower(),
        overlap=get_optional_float(context.run_config, "fedrolex-overlap"),
    )
