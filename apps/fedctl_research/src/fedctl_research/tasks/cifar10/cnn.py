"""CIFAR-10 width-scaled CNN task."""
...
def _scaled_channels(model_rate: float) -> list[int]:
    return [
        ...
        max(1, int(round(128 * model_rate))),
        max(1, int(round(256 * model_rate))),
    ]


def build_model_for_rate(model_rate: float, *, global_model_rate: float = 1.0) -> HeteroCifar10CNN:
    scaler_rate = float(model_rate) / float(global_model_rate)
    return HeteroCifar10CNN(channels=_scaled_channels(model_rate), scaler_rate=scaler_rate)
...
def train(
    ...
    *,
    ...
    return train_classifier(
        ...
        log_prefix=log_prefix or "[cifar10_cnn]",
    )
...
class Cifar10CnnTask:
    ...
    def load_centralized_test_dataset_for_labels(
        ...
        *,
        ...
        return load_centralized_test_dataset_for_labels(
            labels,
