"""California Housing width-scaled MLP task."""
...
from fedctl_research.tasks.california_housing.data import (
...
def _scaled_hidden(model_rate: float, base_width: int) -> int:
    return max(8, int(round(base_width * model_rate)))
...
) -> CaliforniaHousingMLP:
    _ = global_model_rate
    return CaliforniaHousingMLP(
        input_dim=INPUT_DIM,
        hidden1=_scaled_hidden(model_rate, 128),
        hidden2=_scaled_hidden(model_rate, 64),
    )
...
def train(
    ...
    *,
    ...
    return train_regressor(
        ...
        device,
        optimizer=optimizer,
        log_prefix=log_prefix or "[california_housing_mlp]",
    )
...
class CaliforniaHousingMlpTask:
    name: str = "california_housing_mlp"
