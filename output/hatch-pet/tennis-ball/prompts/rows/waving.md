Create a single horizontal sprite strip for the Codex app digital pet `tennis-ball` in the state `waving`.
...
Output exactly 4 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: a fuzzy neon-yellow tennis ball mascot with curved white seam markings, tiny arms and feet, and a simple expressive face.
