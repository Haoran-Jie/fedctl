...
RAW_FILENAME = "network_main_accuracy_vs_wall_clock_raw.csv"
SUMMARY_FILENAME = "network_main_accuracy_vs_wall_clock_summary.csv"
MAX_SECONDS = 4500


def main() -> None:
    points, summaries = fetch_or_load(RAW_FILENAME, SUMMARY_FILENAME)
    curve_rows = aggregate_curve(points, x_axis="wall_clock")
    target_rows = aggregate_targets(summaries)
    diagnostic_rows = aggregate_diagnostics(summaries)
    write_curve_aggregate("network_main_accuracy_vs_wall_clock_aggregate.csv", curve_rows)
    ...
    for spec in run_specs():
        run_points = [p for p in points if p.run_id == spec.run_id and p.wall_clock_s is not None]
    ...
    fig, axes = plt.subplots(2, 2, figsize=(PUBLICATION_FIGURE_WIDTH, 6), sharex=True, sharey=False)
        ...
        for col_idx, topology in enumerate(TOPOLOGY_ORDER):
            ...
            for method in METHOD_ORDER:
                ...
                if target and not target.target_censored:
                    ax.scatter(
                        [target.target_wall_clock_s_mean],
                        [TARGET_ACC],
                        s=62,
                        marker="*",
                        color=colors[method],
    ...
    fig.legend(by_label.values(), by_label.keys(), loc="upper center", ncol=4, frameon=True, bbox_to_anchor=(0.5, 1.02))
