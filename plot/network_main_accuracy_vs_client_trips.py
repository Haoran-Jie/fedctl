...
def _fetch_run(api: wandb.Api, spec: RunSpec) -> tuple[list[EvalPoint], SummaryRow]:
    ...
    for row in run.scan_history(page_size=1000):
        trip = _client_trip_from_row(row)
        score = _score_from_row(row)
        if trip is None or score is None:
            continue
        key = (trip, score)
        if key in seen:
            continue
    ...
    points.sort(key=lambda p: p.client_trip)
...
def _load_cached() -> tuple[list[EvalPoint], list[SummaryRow]]:
    points_path = plot_output_path("network_main_accuracy_vs_client_trips_raw.csv")
    summary_path = plot_output_path("network_main_accuracy_vs_client_trips_summary.csv")
        ...
        for row in csv.DictReader(f):
            ...
            points.append(
                EvalPoint(
                    ...
                    method=row["method"],
                    run_id=row["run_id"],
                    client_trip=int(row["client_trip"]),
                    eval_acc=float(row["eval_acc"]),
                    server_step=int(server_step) if server_step else None,
                )
            )
...
def _write_cache(points: list[EvalPoint], summaries: list[SummaryRow]) -> None:
    write_csv_plot(
        "network_main_accuracy_vs_client_trips_raw.csv",
        ["regime", "topology", "method", "run_id", "client_trip", "eval_acc", "server_step"],
        (
            [p.regime, p.topology, p.method, p.run_id, p.client_trip, p.eval_acc, p.server_step or ""]
            for p in points
        ),
    )
    write_csv_plot(
        "network_main_accuracy_vs_client_trips_summary.csv",
        [
            "regime",
            "topology",
            "method",
        ...
        coverage.append(
            {
                ...
                "first_trip": min((p.client_trip for p in run_points), default=None),
                "last_trip": max((p.client_trip for p in run_points), default=None),
    ...
    for row_idx, regime in enumerate(REGIME_ORDER):
        for col_idx, topology in enumerate(TOPOLOGY_ORDER):
            ...
            for method in METHOD_ORDER:
                ...
                if not subset:
                    continue
                xs = np.array([p.client_trip for p in subset], dtype=float)
                ...
                if summary and summary.target_reached and summary.target_client_trips is not None:
                    y_at_target = next((p.eval_acc for p in subset if p.client_trip == summary.target_client_trips), None)
                    if y_at_target is not None:
                        ax.scatter(
                            [summary.target_client_trips],
                            [y_at_target],
                            s=62,
                            marker="*",
                            color=colors[method],
