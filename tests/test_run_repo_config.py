...
from fedctl.commands.run import _resolve_run_deploy_config


def test_resolve_run_deploy_config_prefers_explicit_value(tmp_path: Path) -> None:
    project_root = tmp_path / "proj"
    project_root.mkdir()
    explicit = str(tmp_path / "external-fedctl.yaml")

    resolved = _resolve_run_deploy_config(
        deploy_config=explicit,
...
def test_resolve_run_deploy_config_uses_project_local_config(tmp_path: Path) -> None:
    project_root = tmp_path / "proj"
...
def test_resolve_run_deploy_config_returns_none_when_not_found(tmp_path: Path) -> None:
    project_root = tmp_path / "proj"
    project_root.mkdir()
