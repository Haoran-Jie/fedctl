...
from fedctl_research.tasks.california_housing.data import DOWNLOAD_LOCK, NPZ_PATH, datasets_pair
from fedctl_research.tasks.california_housing.mlp import build_model_for_rate


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a single centralized California Housing MLP experiment with tqdm progress."
    )
    ...
    parser.add_argument(
        "--output",
        default=str(REPO_ROOT / "tmp" / "california_housing_single_run_tqdm.csv"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="Remove the dataset lock if it is older than this and the npz is still missing.",
    )
    return parser.parse_args()


def clear_stale_download_lock(max_age_seconds: int) -> None:
    if NPZ_PATH.exists() or not DOWNLOAD_LOCK.exists():
        return
...
def main() -> int:
    ...
    if NPZ_PATH.exists():
        print(f"[single-run] using cached dataset at {NPZ_PATH}", flush=True)
    else:
        print(f"[single-run] dataset not cached yet; loader will download to {NPZ_PATH}", flush=True)
