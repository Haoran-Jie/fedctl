...
def _netem_script() -> str:
    lines = ["set -eu", *_netem_setup_lines(), *_netem_verification_lines()]
    lines.extend(
        [
...
def _netem_setup_lines() -> list[str]:
    return [
...
def _netem_verification_lines() -> list[str]:
    return [
        'export NET_EFFECTIVE_IFACE="$IFACE"',
        'export NET_EFFECTIVE_INGRESS_IFACE="$IN_IFACE"',
        'export NET_EFFECTIVE_INGRESS_IFB="$IN_IFB"',
        "python - <<'PY' || true",
        "import json",
        "import os",
        "import re",
        ...
        "",
        "PREFIX = '[netem-json]'",
        "QDISC_RE = re.compile(r'\\bqdisc\\b.*\\b(netem|tbf)\\b')",
        ...
        "        return None",
        "",
        "def to_mbit(value: str, unit: str) -> float:",
        "    multipliers = {'K': 0.001, 'M': 1.0, 'G': 1000.0}",
        "    return float(value) * multipliers[unit.upper()]",
        "",
        "def read_qdisc_lines(device: str) -> list[str]:",
        "    if not device:",
        "        return []",
        "    proc = subprocess.run(['tc', 'qdisc', 'show', 'dev', device], capture_output=True, text=True, check=False)",
        "    text = '\\n'.join(part for part in (proc.stdout, proc.stderr) if part)",
        "    return [line.strip() for line in text.splitlines() if line.strip()]",
        "",
        "def parse_metrics(lines: list[str]) -> dict[str, object]:",
        "    payload = {",
        "        'qdisc_applied': False,",
        "        'delay_ms_applied': None,",
        "        'jitter_ms_applied': None,",
        "        'loss_pct_applied': None,",
        "        'rate_mbit_applied': None,",
        "    }",
        "    for line in lines:",
        "        if not QDISC_RE.search(line):",
        "            continue",
        "        payload['qdisc_applied'] = True",
        "        delay = DELAY_RE.search(line)",
        "        loss = LOSS_RE.search(line)",
        "        rate = RATE_RE.search(line)",
        "        if payload['delay_ms_applied'] is None and delay:",
        "            payload['delay_ms_applied'] = float(delay.group(1))",
        "        if payload['jitter_ms_applied'] is None and delay and delay.group(2):",
        "            payload['jitter_ms_applied'] = float(delay.group(2))",
        "        if payload['loss_pct_applied'] is None and loss:",
        "            payload['loss_pct_applied'] = float(loss.group(1))",
        "        if payload['rate_mbit_applied'] is None and rate:",
        "            payload['rate_mbit_applied'] = to_mbit(rate.group(1), rate.group(2))",
        "    return payload",
        "",
        "egress_profile = os.environ.get('NET_PROFILE', 'none')",
        "ingress_profile = os.environ.get('NET_INGRESS_PROFILE', 'none')",
        "directions = [",
        "    {",
        "        'direction': 'egress',",
        "        'iface': os.environ.get('NET_EFFECTIVE_IFACE', os.environ.get('NET_IFACE', 'eth0')),",
        "        'source_iface': os.environ.get('NET_EFFECTIVE_IFACE', os.environ.get('NET_IFACE', 'eth0')),",
        "        'enabled': egress_profile != 'none',",
        "        'observed_profile': egress_profile,",
        "        'delay_ms_expected': env_float('NET_DELAY_MS'),",
        "        'jitter_ms_expected': env_float('NET_JITTER_MS'),",
        "        'loss_pct_expected': env_float('NET_LOSS_PCT'),",
        "        'rate_mbit_expected': env_float('NET_RATE_MBIT'),",
        "    },",
        "    {",
        "        'direction': 'ingress',",
        "        'iface': os.environ.get('NET_EFFECTIVE_INGRESS_IFB', os.environ.get('NET_INGRESS_IFB', 'ifb0')),",
        "        'source_iface': os.environ.get('NET_EFFECTIVE_INGRESS_IFACE', os.environ.get('NET_EFFECTIVE_IFACE', os.environ.get('NET_IFACE', 'eth0'))),",
        "        'enabled': os.environ.get('NET_INGRESS_ENABLED') == '1' and ingress_profile != 'none',",
        "        'observed_profile': ingress_profile,",
        "        'delay_ms_expected': env_float('NET_INGRESS_DELAY_MS'),",
        "        'jitter_ms_expected': env_float('NET_INGRESS_JITTER_MS'),",
        "        'loss_pct_expected': env_float('NET_INGRESS_LOSS_PCT'),",
        "        'rate_mbit_expected': env_float('NET_INGRESS_RATE_MBIT'),",
        "    },",
        "]",
        "",
        "for item in directions:",
        "    raw_lines = read_qdisc_lines(str(item['iface']))",
        "    payload = {",
        "        'event': 'verify',",
        "        'direction': item['direction'],",
        "        'iface': item['iface'],",
        "        'source_iface': item['source_iface'],",
        "        'enabled': item['enabled'],",
        "        'observed_profile': item['observed_profile'],",
        "        'expected_egress_profile': egress_profile,",
        "        'expected_ingress_profile': ingress_profile,",
        "        'delay_ms_expected': item['delay_ms_expected'],",
        "        'jitter_ms_expected': item['jitter_ms_expected'],",
        "        'loss_pct_expected': item['loss_pct_expected'],",
        "        'rate_mbit_expected': item['rate_mbit_expected'],",
        "        'raw_lines': raw_lines,",
        "        **parse_metrics(raw_lines),",
        "    }",
        "    print(f\"{PREFIX} {json.dumps(payload, separators=(',', ':'))}\")",
        "PY",
    ]
