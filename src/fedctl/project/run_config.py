...
_ARCHIVED_RUN_CONFIG = Path(".fedctl") / "run_config.toml"
...
_SECTION_KEY_MAP: dict[str, dict[str, str]] = {
    ...
    "fedbuff": {
        ...
        "server-learning-rate": "fedbuff-server-learning-rate",
        ...
        "fedbuff-server-learning-rate": "fedbuff-server-learning-rate",
        "fedbuff-staleness-weighting": "fedbuff-staleness-weighting",
        "fedbuff-staleness-alpha": "fedbuff-staleness-alpha",
    },
    "fedcover": {
        "slicer": "fedcover-slicer",
        "base-staleness-weighting": "fedcover-base-staleness-weighting",
        "staleness-alpha": "fedcover-staleness-alpha",
        "buffer-size": "fedcover-buffer-size",
        "train-concurrency": "fedcover-train-concurrency",
        "poll-interval-s": "fedcover-poll-interval-s",
        "num-server-steps": "fedcover-num-server-steps",
        "evaluate-every-steps": "fedcover-evaluate-every-steps",
        "server-learning-rate": "fedcover-server-learning-rate",
        "coverage-power": "fedcover-coverage-power",
        "max-block-weight": "fedcover-max-block-weight",
        "min-observed-mass": "fedcover-min-observed-mass",
        "fedcover-slicer": "fedcover-slicer",
        "fedcover-base-staleness-weighting": "fedcover-base-staleness-weighting",
        "fedcover-staleness-alpha": "fedcover-staleness-alpha",
        "fedcover-buffer-size": "fedcover-buffer-size",
        "fedcover-train-concurrency": "fedcover-train-concurrency",
        "fedcover-poll-interval-s": "fedcover-poll-interval-s",
        "fedcover-num-server-steps": "fedcover-num-server-steps",
        "fedcover-evaluate-every-steps": "fedcover-evaluate-every-steps",
        "fedcover-server-learning-rate": "fedcover-server-learning-rate",
        "fedcover-coverage-power": "fedcover-coverage-power",
        "fedcover-max-block-weight": "fedcover-max-block-weight",
        "fedcover-min-observed-mass": "fedcover-min-observed-mass",
    },
    "fiarse": {
...
class RunConfigResolution:
    resolved_path: Path
    runner_path: str
    archive_source: Path | None


def resolve_run_config_input(
    project_root: Path,
    run_config: str | None,
) -> Path | None:
    if not run_config:
    ...
    raw = Path(run_config).expanduser()
    candidates = []
    if raw.is_absolute():
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise ProjectError(f"Run config not found: {run_config}")


def resolve_run_config(
    project_root: Path,
    run_config: str | None,
) -> RunConfigResolution | None:
    resolved = resolve_run_config_input(project_root, run_config)
    if resolved is None:
        return None

    normalized_path = _normalize_run_config(resolved)
    normalized_source = normalized_path or resolved
    project_root_resolved = project_root.resolve()
    try:
        rel = resolved.relative_to(project_root_resolved)
        return RunConfigResolution(
        ...
        return RunConfigResolution(
            resolved_path=normalized_source,
            runner_path=str(_ARCHIVED_RUN_CONFIG),
...
def extract_seed_sweep(
    ...
    if "run" in data and isinstance(data["run"], dict):
        raw = data["run"].get("seeds")
    else:
        raw = data.get("seeds")
    if raw is None:
        return ()
    if not isinstance(raw, list) or not raw:
        raise ProjectError("Run config seed sweep must be a non-empty array")
...
def _normalize_run_config(path: Path) -> Path | None:
    data = tomllib.loads(path.read_text(encoding="utf-8"))
    normalized, changed = _flatten_run_config(data)
    if not changed:
        return None
    return _write_temp_toml(normalized, prefix="fedctl-run-config-")


def _flatten_run_config(data: dict[str, object]) -> tuple[dict[str, object], bool]:
...
def _flatten_section(normalized: dict[str, object], section: str, value: dict[str, object]) -> None:
    if section in _SECTION_KEY_MAP:
        ...
        for key, item in value.items():
            if section == "run" and key == "seeds":
                continue
            flat_key = mapping.get(key)
            if flat_key is None:
                raise ProjectError(
                    f"Unsupported key in run config section [{section}]: {key}"
                )
    ...
    if section == "devices":
        for device_name, device_values in value.items():
            if not isinstance(device_values, dict):
                raise ProjectError("Expected [devices.<device>] tables in run config")
            for key, item in device_values.items():
                suffix = _DEVICE_VALUE_MAP.get(key)
                if suffix is None:
                    raise ProjectError(
                        f"Unsupported key in run config section [devices.{device_name}]: {key}"
                    )
                normalized[f"{device_name}-{suffix}"] = _normalize_scalar_value(item)
        return

    raise ProjectError(f"Unsupported run config section: [{section}]")
