...
\begin{description}[leftmargin=2.8cm, style=nextline]
  ...
  \item[Constraint] A scheduling rule. Typed client placement uses constraints such as matching a requested worker type to Nomad node metadata.
...
This extra runner layer keeps the user's laptop out of the critical path after submission. Once a run has been accepted, queueing, deployment, logging, and artefact capture proceed on the cluster side.
...
\begin{definition}
\textbf{W\&B and structured artefacts.}
Weights \& Biases (W\&B) stores experiment metrics and run summaries. Structured artefacts are files emitted by the research application, such as event-level asynchronous-update logs and per-client diagnostic rows. Together with submit-service logs, they form the evidence path used to regenerate tables, figures, and failure diagnostics.
\end{definition}
%TC:endignore
