...
This chapter establishes the background. Section~\ref{sec:problem_setting} introduces FL through the global objective and the synchronous \texttt{FedAvg} training pipeline, which serves as the reference for later comparisons. Section~\ref{sec:heterogeneity_problems} defines the three forms of heterogeneity under realistic edge FL: data heterogeneity changes clients' data distributions; compute heterogeneity changes clients' local training capabilities; and straggler-induced heterogeneity changes update timing, staleness, and influence.

\section{Federated Learning}
\label{sec:problem_setting}

Federated learning (FL) enables clients to collaboratively train a model without centralising their data \parencite{mcmahan2017fedavg,kairouz2021advances,bonawitz2019towards}.\footnote{Decentralised training does not itself guarantee privacy: deployed FL is often combined with secure aggregation, which hides individual client updates while allowing the server to recover their aggregate, or differential privacy, which adds calibrated noise to FL updates to limit the information that can be inferred about any client \parencite{bonawitz2017secureaggregation,dwork2006calibrating,abadi2016deep}. These privacy mechanisms are orthogonal to the systems heterogeneity studied here.} Let \(\mathcal{K}\) denote the client set, \(D_k\) the local dataset held by client \(k\), \(n_k=|D_k|\), \(n=\sum_{j\in\mathcal{K}}n_j\), and \(w\) the global model parameters. The FL objective is\footnote{The mathematical symbols and notation used throughout the dissertation are summarised in Appendix~\ref{chap:notation}.}
\[
    \min_w F(w),
    \qquad
    F(w) = \sum_{k \in \mathcal{K}} \frac{n_k}{n} F_k(w),
    \qquad
    F_k(w) = \frac{1}{n_k}\sum_{(x,y)\in D_k} \ell(w;x,y).
\]
Here, \(\ell\) is the per-example loss.
In a synchronous \texttt{FedAvg} round, the server sends \(w^{(t)}\) to the selected client set \(\mathcal{C}_t\), each client performs local optimisation, and the server aggregates the returned models:
\[
    \tilde{w}_k^{(t+1)}
    =
...
\node[axis, draw=darkblue!70!black, fill=darkblue!7, text=darkblue!70!black, minimum width=4.70cm, text width=4.25cm]
  at (5.85,-3.40) {straggler-induced: update timing and staleness};
...
\node[
  ...
  at (-3,-1.72)
  {\textcolor{paletteOrange!90!black}{\(F(w)=\sum_k (n_k/n)F_k(w)\) is estimated without centralising raw data}};
\end{tikzpicture}
\end{adjustbox}
...
This abstraction describes how models are trained and aggregated, but it hides many of the systems effects, including client selection, local training time, communication, and aggregation. In homogeneous settings or under simulation, these effects may be negligible. On heterogeneous settings they become hard to ignore as they often lead to the bottleneck in the deployed system: how long a round takes, which client is available, how much influence a client has on the global model.

Figure~\ref{fig:background_fl_round} makes this concrete by connecting the FL training pipeline to the three forms of heterogeneity defined in Section~\ref{sec:heterogeneity_problems} and shows where they enter the evaluation.

\FloatBarrier
\section{Heterogeneity in Federated Learning}
\label{sec:heterogeneity_problems}

Realistic edge-FL deployments are heterogeneous in several ways. Clients collect different data, expose different processor and memory budgets, and return updates through networks with variable latency, bandwidth, and availability \parencite{kairouz2021advances,wang2021field,bonawitz2019towards,lai2022fedscale}. Treating these effects as one generic ``heterogeneity'' problem hides and mixes the underlying mechanisms. This dissertation separates them into \textbf{data heterogeneity, compute heterogeneity, and straggler-induced heterogeneity} so that each method family is evaluated against the heterogeneity it is designed to address.
...
\end{tikzpicture}
\end{adjustbox}
\caption{\textbf{Three forms of heterogeneity used in this dissertation.}}
\label{fig:background_heterogeneity_forms}
\end{figure}

\subsection{Data Heterogeneity}

Data heterogeneity means that clients' local datasets are non-IID. Formally, \(D_k \not\sim D_j\) for some clients \(k,j\). Therefore, the local objectives \(F_k\) can point in different optimisation directions. This is a standard FL challenge \parencite{kairouz2021advances,wang2021field}. It can appear as label skew, where clients observe different class proportions; feature skew, where the input distribution differs across clients; or device-correlated skew, where the data distribution is correlated with hardware class. In this dissertation, it is included because it interacts with systems heterogeneity: clients may have distinct data distributions because hardware capabilities can correlate with where and how data are collected. The evaluation therefore explores both IID and non-IID partitioning, and includes a device-correlated partitioning ablation to test whether stale-aware weighting can compensate when data distributions are correlated with client capabilities.

\subsection{Compute Heterogeneity}
\label{sec:background_compute_heterogeneity}

Compute heterogeneity refers to differences in client compute capabilities, including processor speed, memory capacity, and the batch size or model size constraints \parencite{li2020fedprox,lai2022fedscale}. The evaluation represents this with mixed \texttt{rpi4} and \texttt{rpi5} workers. Under real deployments, the problem is not only that weaker devices perform each local step more slowly, but also that for some model--device pairs, holding the full model may be infeasible due to limited memory. A per-client time and memory constraint can be:
\[
    T_k^{\mathrm{train}}(m, B_k, E)
    \approx
    \frac{
      E\left\lceil n_k / B_k \right\rceil \Phi(m, B_k)
    }{\rho_k},
    \qquad
    M_k(m,B_k) \le \bar{M}_k,
\]
where \(m\) is the trainable model, \(B_k\) the local batch size, \(E\) the number of local epochs, \(\Phi(m,B_k)\) the per-batch training work, \(\rho_k\) the client's effective throughput, \(M_k(m,B_k)\) the memory required to store the model and its gradients, and \(\bar{M}_k\) the memory budget. 

A weaker client has lower throughput \(\rho_k\) and memory budget \(\bar{M}_k\), which increases local training time \(T_k^{\mathrm{train}}\) and restricts feasible model sizes \(M_k\). Submodel FL resolves this by assigning smaller subnetworks to weaker clients \parencite{diao2021heterofl,horvath2021fjord,alam2022fedrolex,wu2024fiarse}. Let \(r_k\in(0,1]\) denote client \(k\)'s model rate. The server sends only the parameter subset indexed by \(I_k^{(t)}\) to the client:
\[
    w_{k,0}^{(t)} = w^{(t)}_{I_k^{(t)}},
    \qquad
    \frac{|I_k^{(t)}|}{|w^{(t)}|} \approx r_k.
\]
This not only gives faster updates from weaker clients, but also lets the assigned submodel fit within memory constraints that a full model would exceed. This allows valuable client data to contribute to the global model while also producing useful, deployable local submodels for inference.

\subsection{Straggler-Induced Heterogeneity}
\label{sec:background_straggler_heterogeneity}

Straggler-induced heterogeneity arises due to variations in client completion times. A client update has an end-to-end completion time
\[
    C_k^{(t)}
    =
    T_{k,\mathrm{down}}^{(t)}
    ...
    T_{k,\mathrm{queue}}^{(t)},
\]
combining model download, local training, upload, and deployment/runtime overhead. Network conditions change the communication terms through delay, jitter, loss, bandwidth, and asymmetry. Synchronous FL waits for all the selected clients in each round, so the completion time is determined by the slowest client. Asynchronous FL avoids that issue by allowing clients to respond at different times. However, this introduces \textbf{stale updates}: if a client trained on server version \(v_i\) and the server is now at step \(s\), then \textbf{staleness \(\tau\)} is:
\[
    \tau_i = s - v_i.
\]
